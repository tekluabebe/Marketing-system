...
<div id="body">
    ...
    <div class="rcontent">
	  ...
	  if(!strcmp(strtolower($_GET['name']),"product") & isset($_GET['success'])){
				$result=mysqli_query($connect,"update product set product_name='{$_POST['product_name']}',supplier_id={$_POST['supplier']},quantity={$_POST['quantity']},product_type='{$_POST['product_type']}',market_price={$_POST['mprice']} where product_id='{$_POST['product_id']}'");
				...
				while($row = mysqli_fetch_array($dept_set))
				 ...
				 while($row = mysqli_fetch_array($supplier_set))
					if($row['sid']==$plist['supplier_id']) echo "<option value='{$row['sid']}' selected='selected'>{$row['sname']}</option>";
						...
						echo"</select></td></tr>
						<tr><td style='padding:5px'>Quantity: </td><td><input name='quantity' type='text' required value='{$plist['quantity']}' /></td></tr>
	  ...
	  //supplier start
	  	  if(!strcmp(strtolower($_GET['name']),"supplier") & isset($_GET['success'])){
				$result=mysqli_query($connect,"update supplier set sname='{$_POST['name']}',saddress='{$_POST['address']}',sdealer='{$_POST['dealer']}',sphone={$_POST['phone']},semail='{$_POST['email']}' where sid='{$_POST['sid']}'");
	   ...
	   if(isset($_GET['name'])&isset($_GET['id'])){
		...
		if(!strcmp(strtolower($_GET['name']),"supplier")){
			...
			echo"<div id='data'>";
				...
				$plist=mysqli_fetch_array($plist);
			echo "<form method='post' action='editlist.php?name=suppliers&success=1'>
					  <table>
	  ...
	  //customer start
	  	  if(!strcmp(strtolower($_GET['name']),"customer") & isset($_GET['success'])){
				$result=mysqli_query($connect,"update customer set first_name='{$_POST['fname']}',last_name='{$_POST['lname']}',caddress='{$_POST['caddress']}',cphone={$_POST['cphone']} where cid='{$_POST['cid']}'");
				if(!$result)echo "Addition not successful ".mysql_error();
	   ...
	   if(isset($_GET['name'])&isset($_GET['id'])){
		...
		if(!strcmp(strtolower($_GET['name']),"customer")){
			...
			echo"<div id='data'>";
				...
				$plist=mysqli_query($connect,"SELECT * from customer where cid='{$_GET['id']}'");
				$plist=mysqli_fetch_array($plist);
